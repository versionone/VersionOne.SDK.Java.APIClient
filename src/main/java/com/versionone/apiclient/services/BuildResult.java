...
public class BuildResult {
    ...
    public String toUrl() {
        if (logger.isDebugEnabled()) {
            ...
            String url = path.concat(querystring != null ? "?" + querystring : "");
            logger.debug(format("Raw URL: %s", url));
        }
        String path = TextBuilder.join(pathParts, "/");
        ...
        String encodedUrl = path.concat(querystring != null ? "?" + querystring : "");

        logger.debug(format("Encoded URL : %s", encodedUrl));
